...
public class BookController {
    ...
    @PostMapping
    public BookResponseDTO create(final @RequestBody BookUpdateDTO bookUpdateDTO) {
        return toBookResponseDTO(bookService.create(toBook(bookUpdateDTO)));
    }

    @Operation(summary = "Update a specific book")
    @PutMapping("{id}")
    public BookResponseDTO update(final @PathVariable UUID id, final @RequestBody BookUpdateDTO bookUpdateDTO) {
        return toBookResponseDTO(bookService.update(id, toBook(bookUpdateDTO)));
    }
